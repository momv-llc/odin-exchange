        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navigation.map(item => {
              ...
              return (
                <Link key={item.name} to={item.href} className={`flex items-center space-x-2 text-slate-300 hover:text-emerald-400 transition-colors ${isActive(item.href) ? 'text-emerald-400' : ''}`}>
          ...
          <div className="hidden md:flex items-center space-x-4">
            <select value={currentLang} onChange={e => setCurrentLang(e.target.value as Language)} className="bg-slate-700/50 text-slate-300 px-3 py-2 rounded-lg border border-slate-600/50 focus:outline-none focus:border-emerald-400">
              <option value="en">EN</option>
              <option value="de">DE</option>
              <option value="ru">RU</option>
              <option value="ua">UA</option>
            </select>
            {AuthButtons ? <AuthButtons /> : renderAuthButton()}
          </div>
        ...
        <div className="md:hidden bg-slate-900/95 backdrop-blur-xl border-t border-slate-700/50">
          <div className="px-4 py-4 space-y-2">
            {navigation.map(item => {
              const Icon = item.icon;
              return (
                <Link key={item.name} to={item.href} onClick={() => setIsMenuOpen(false)} className={`flex items-center space-x-3 px-4 py-3 rounded-lg text-slate-300 hover:text-emerald-400 hover:bg-slate-800/50 transition-colors ${isActive(item.href) ? 'text-emerald-400 bg-slate-800/50' : ''}`}>
            ...
            <div className="pt-4 border-t border-slate-700/50 space-y-3">
              <select value={currentLang} onChange={e => setCurrentLang(e.target.value as Language)} className="w-full bg-slate-700/50 text-slate-300 px-4 py-3 rounded-lg border border-slate-600/50 focus:outline-none focus:border-emerald-400">
                <option value="en">EN</option>
                <option value="de">DE</option>
                <option value="ru">RU</option>
                <option value="ua">UA</option>
              </select>
              <div onClick={() => setIsMenuOpen(false)}>{AuthButtons ? <AuthButtons /> : renderAuthButton()}</div>
            </div>
          </div>
        </div>
      )}
